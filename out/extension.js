...
// src/panels/views/LoadingPage.ts
...
// src/utilities/paletteLogin.ts
var childProcess = __toESM(require("child_process"));
function paletteLogin(data) {
  let error = "";
  try {
    childProcess.execSync(`palette pde login --api-key ${data[0]} --console-url ${data[1]} --project Default`);
    return { error, status: "loginSuccess" };
  } catch (error2) {
    return { error: error2, status: "loginFailed" };
  }
}

// src/panels/views/SettingsPage.ts
var fs = __toESM(require("fs"));
var os = __toESM(require("os"));
function getSettingsPage(projectList, clusterGroupList, clusterInfo, selectedRadio, isDisabled) {
  return `
      <body>
...
function getTableString(clusterInfo, selectedRadio) {
  let tableString = `
...
function getSelections(option, iterable) {
  let homedir6 = os.homedir();
  let paletteFile = fs.readFileSync(
    `${homedir6}/.palette/pde/pde.yaml`,
    "utf-8"
  );
  let pdeyaml = load(paletteFile);
  ...
  for (let i = 0; i < iterable.length; i++) {
    if (selected == iterable[i]) {
      html = html + `<vscode-option value="${iterable[i]}" selected>${iterable[i]}</vscode-option>`;
    } else {
...
// src/utilities/getClusterGroupList.ts
var childProcess2 = __toESM(require("child_process"));
function getClusterGroupList() {
  let error = "";
  try {
    let out = childProcess2.execSync(`palette pde cluster-group list`);
    ...
    for (let i = 0; i < clusterGroupList.length; i++) {
      ...
      } else if (clusterGroupList[i].includes("\n") && i != clusterGroupList.length - 1) {
        list.push(clusterGroupList[i].split("\n")[1]);
    ...
    return { error, clusterGroupList: list, scopList };
  } catch (error2) {
    return { error: error2, clusterGroupList: [], scopList: [] };
  }
}

// src/utilities/getProjList.ts
var childProcess3 = __toESM(require("child_process"));
function getProjectList() {
  let error = "";
  try {
    let out = childProcess3.execSync(`palette pde project list`);
    let pStdout = out.toString();
    ...
    return { error, projectList: list };
  } catch (error2) {
    return { error: error2, projectList: [] };
  }
}

// src/utilities/switchConfig.ts
var childProcess4 = __toESM(require("child_process"));
function switchConfig(data, option) {
  let error = "";
  try {
    childProcess4.execSync(`palette pde ${option} switch --name ${data}`);
    return error;
  } catch (error2) {
    return error2;
  }
...
// src/utilities/getClusterList.ts
var childProcess5 = __toESM(require("child_process"));
function getClusterList() {
  let error = "";
  try {
    let out = childProcess5.execSync(`palette pde virtual-cluster list`);
    ...
    return { error, finalrow };
  } catch (error2) {
    return { error: error2, finalrow: [] };
  }
}

// src/utilities/downloadKube.ts
var childProcess6 = __toESM(require("child_process"));
var fs2 = __toESM(require("fs"));
function downloadKube(data) {
  let error = "";
  let success = "";
  let status = 0;
  try {
    let out = childProcess6.execSync(`palette pde virtual-cluster download-kubeconfig --name ${data}`);
    let pStdout = out.toString();
    let kcPath = " ";
    let newStr = [];
    kcPath = pStdout.split(" ").at(-1);
    for (let i = 0; i < kcPath.length - 1; i++) {
      newStr.push(kcPath[i]);
    }
    kcPath = newStr.map((item) => item).join("");
    const kc = fs2.readFileSync(`${kcPath}`, "utf-8");
    if (kc.length == 0) {
      success = "The cluster is still provisioning. Please try again later.";
    } else {
      success = pStdout;
      status = 1;
    }
    ...
    return { error: error2, success, status };
  }
}
...
// src/utilities/deleteCluster.ts
var childProcess7 = __toESM(require("child_process"));
function deleteCluster(data) {
  let error = "";
  let success = "";
  try {
    let out = childProcess7.execSync(`palette pde virtual-cluster delete --name  ${data}`);
    success = out.toString();
    return { error, success };
  } catch (error2) {
    return { error: error2, success };
  }
}

// src/panels/views/CreationPage.ts
var fs3 = __toESM(require("fs"));
var os2 = __toESM(require("os"));
function getCreationPage() {
  let homedir6 = os2.homedir();
  let paletteFile = fs3.readFileSync(`${homedir6}/.palette/pde/pde.yaml`, "utf-8");
  let pdeyaml = load(paletteFile);
  return `
      <body>
...
// src/utilities/createCluster.ts
var childProcess8 = __toESM(require("child_process"));
var fs4 = __toESM(require("fs"));
var os3 = __toESM(require("os"));
function createCluster(data, scope) {
  let error = "";
  try {
    let homedir6 = os3.homedir();
    let paletteFile = fs4.readFileSync(
      `${homedir6}/.palette/pde/pde.yaml`,
      "utf-8"
    );
    let pdeyaml = load(paletteFile);
    ...
    let storage = data[3];
    let memory = data[2];
    let out = childProcess8.execSync(`palette pde virtual-cluster create --cpu ${cpu} --memory ${memory} --name ${name} --storage ${storage} --cluster-group-name ${pdeyaml.ClusterGroupName} --cluster-group-scope ${scope}`);
    let pStdout = out.toString();
    return { error, success: pStdout, status: 1 };
  } catch (error2) {
    return { error: error2, success: "", status: 0 };
  }
}

// src/panels/views/ResizePage.ts
var fs5 = __toESM(require("fs"));
var os4 = __toESM(require("os"));
function getResizePage(clusterName) {
  let homedir6 = os4.homedir();
  let paletteFile = fs5.readFileSync(`${homedir6}/.palette/pde/pde.yaml`, "utf-8");
  let pdeyaml = load(paletteFile);
  return `
      <body>
        <div class="creationPage">
...
// src/utilities/resizeCluster.ts
var childProcess9 = __toESM(require("child_process"));
function resizeCluster(data, name) {
  let error = "";
  try {
    let cpu = data[0];
    let storage = data[2];
    let memory = data[1];
    let out = childProcess9.execSync(`palette pde virtual-cluster resize --cpu ${cpu} --name ${name} --memory ${memory} --storage ${storage}`);
    let pStdout = out.toString();
    return { error, success: pStdout, status: 1 };
  } catch (error2) {
    return { error: error2, success: "", status: 0 };
...
// src/utilities/pauseCluster.ts
var childProcess10 = __toESM(require("child_process"));
function pauseCluster(name) {
  let error = "";
  try {
    let out = childProcess10.execSync(`palette pde virtual-cluster lifecycle --name  ${name} --action pause`);
    let pStdout = out.toString();
    return { error, success: pStdout };
  } catch (error2) {
    return { error: error2, success: "" };
  }
}

// src/utilities/resumeCluster.ts
var childProcess11 = __toESM(require("child_process"));
function resumeCluster(name) {
  let error = "";
  try {
    let out = childProcess11.execSync(`palette pde virtual-cluster lifecycle --name  ${name} --action resume`);
    let pStdout = out.toString();
    return { error, success: pStdout };
  } catch (error2) {
    return { error: error2, success: "" };
...
var HelloWorldPanel = class {
  constructor(panel, extensionUri) {
    ...
    this.homedir = os5.homedir();
  }
  static render(extensionUri) {
    if (HelloWorldPanel.currentPanel) {
      HelloWorldPanel.currentPanel._panel.reveal(import_vscode2.ViewColumn.One);
    } else {
  ...
  dispose() {
    HelloWorldPanel.currentPanel = void 0;
    this._panel.dispose();
  ...
  _getWebviewContent(webview, extensionUri, sendingStep) {
    ...
    switch (sendingStep) {
      ...
      case "loading":
        ...
        break;
      case "login":
        htmlString = htmlString + getSettingsPage(
          this.projectList,
          this.clusterGroupList,
          this.clusterInfo,
          this.selectedRadio,
    ...
    htmlString = htmlString + `${this.error != "" ? `<vscode-divider role="separator"></vscode-divider><h3 class="rederr" readonly><p>${this.error}</p></h3></html>` : "</html>"}`;
    htmlString = htmlString + `${this.success != "" ? `<vscode-divider role="separator"></vscode-divider><h3 class="success" readonly><p>${this.success}</p></h3></html>` : "</html>"}`;
    return htmlString;
  }
  _setWebviewMessageListener(webview) {
    webview.onDidReceiveMessage(
      (message) => {
        ...
        switch (command) {
          case "hello":
            if (fs6.existsSync(`${this.homedir}/.palette/pde/pde.yaml`)) {
              this.paletteFile = fs6.readFileSync(`${this.homedir}/.palette/pde/pde.yaml`, "utf-8");
              this.pdeyaml = load(this.paletteFile);
              let proj2 = getProjectList();
              this.error = this.error + proj2.error;
              this.projectList = proj2.projectList;
              let clustGroup2 = getClusterGroupList();
              this.error = this.error + clustGroup2.error;
              this.clusterGroupList = clustGroup2.clusterGroupList;
              this.scopList = clustGroup2.scopList;
              clustObj = getClusterList();
              this.clusterInfo = clustObj.finalrow;
              this.error = clustObj.error;
            ...
            break;
          case "login":
            const res = paletteLogin(message.data);
          ...
          case "changeClusterGroup":
            this.error = switchConfig(message.data, "cluster-group");
            clustObj = getClusterList();
            this.clusterInfo = clustObj.finalrow;
            this.error = clustObj.error;
            ...
            break;
          case "changeProject":
            this.error = switchConfig(message.data, "project");
            clustObj = getClusterList();
            this.clusterInfo = clustObj.finalrow;
            this.error = clustObj.error;
            webview.html = this._getWebviewContent(this._panel.webview, this.extensionUri, "login");
          ...
          case "downloadKube":
            ...
            this.error = downObj.error;
            this.success = downObj.success;
            webview.html = this._getWebviewContent(this._panel.webview, this.extensionUri, "login");
            if (downObj.status == 1) {
              var k8sExtension = vscode.extensions.getExtension(
          ...
          case "submitCreate":
            ...
            this.error = this.error + createObj.error;
            this.success = this.success + createObj.success;
            if (createObj.status == 1) {
              clustObj = getClusterList();
              this.clusterInfo = clustObj.finalrow;
              this.error = this.error + clustObj.error;
              webview.html = this._getWebviewContent(this._panel.webview, this.extensionUri, "login");
            } else {
              webview.html = this._getWebviewContent(this._panel.webview, this.extensionUri, "create");
            }
          ...
          case "submitResize":
            ...
            this.error = this.error + resizeObj.error;
...
function activate(context) {
  const showHelloWorldCommand = import_vscode3.commands.registerCommand("pde-plugin.openWizard", () => {
